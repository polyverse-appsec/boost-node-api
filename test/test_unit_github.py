...
from constants import TARGET_URL, EMAIL, PREMIUM_EMAIL
...
class GitHubUnitTestSuite(unittest.TestCase):
    ...
    def test_retrieve_files_private(self):
        print("Running test: Retrieve all files from a public project")
        signedHeaders = get_signed_headers(PREMIUM_EMAIL)
        response = requests.get(f"{TARGET_URL}/api/user/org123/connectors/github/files?uri=https://github.com/polyverse-appsec/sara", headers=signedHeaders)
        self.assertEqual(response.status_code, 200)
        self.assertIsNotNone(response.json())
        files = response.json()
