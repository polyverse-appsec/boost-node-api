...
class UnitTestSuite(unittest.TestCase):
    ...
    def test_retrieve_file_private_access_repo_path(self):
        ...
        signedHeaders = get_signed_headers("stephen@polyverse.com")
        response = requests.get(f"{self.TARGET_URL}/api/user/org123/connectors/github/file?repo=https://github.com/polyverse-appsec/sara/&path=README.md", headers=signedHeaders)
        self.assertEqual(response.status_code, 200)
