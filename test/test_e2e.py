...
class End2EndTestSuite(unittest.TestCase):
    ...
    def test_task_generator_launch(self):
        ...
        # cleanup resources
        response = requests.delete(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint", headers=headers)
        self.assertTrue(response.status_code == 200 or response.status_code == 404)
        response = requests.delete(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint/generator", headers=headers)
        self.assertTrue(response.status_code == 200 or response.status_code == 404)
        ...
        response = requests.get(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint/generator", headers=headers)
        self.assertEqual(response.status_code, 200)
        if response.json()['status'] != "idle":
            ...
            response = requests.post(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint/generator", json={"status": "idle"}, headers=headers)
            self.assertEqual(response.status_code, 200)
        ...
        response = requests.post(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint/generator", json={"status": "processing"}, headers=headers)
        ...
        response = requests.get(f"{TARGET_URL}/api/user_project/{ORG}/{PUBLIC_PROJECT_NAME}/data/blueprint", headers=headers)
        self.assertEqual(response.status_code, 200)
        data = response.text
